...
async function startServer() {
  ...
  app.get("*", async (req, res, next) => {
    ...
    if (pageContext.errorWhileRendering) {
      ...
      console.log("ERROR", pageContext.errorWhileRendering);
    }
    ...
    if (!httpResponse) {
      ...
      // When this conditions is true preload tags will be duplicated
      if (false) {
        res.send(body.replaceAll(/\/assets\//g, "http://localhost:3000$&"));
        return;
      }
      res.send(body);
    }
